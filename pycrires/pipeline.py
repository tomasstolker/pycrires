...
class Pipeline:
    ...
    def _plot_trace(self, dpr_catg: str) -> None:
        ...
        for file_item in self.header_data[indices]["ORIGFILE"]:
            ...
            with fits.open(f"{self.path}/raw/{file_item}") as hdu_list:
                ...
                plt.tight_layout()
                plt.savefig(
                    f"{self.path}/calib/util_trace/{file_item[:-5]}_trace.png", dpi=300
                )
                plt.clf()
                plt.close()
    ...
    def cal_flat(self, verbose: bool = False, create_sof: bool = True) -> None:
        ...
        for dit_item in unique_dit:
            ...
            if create_sof:
                print(f"\nCreating SOF file for DIT={dit_item}:")
                ...
                if "CAL_WAVE_TW" in self.file_dict:
                    for key in self.file_dict["CAL_WAVE_TW"]:
                        if not file_found:
                            file_name = key.split("/")[-2:]
                            print(
                                f"   - calib/{file_name[-2]}/{file_name[-1]} CAL_WAVE_TW"
                            )
                ...
                if "CAL_FLAT_TW" in self.file_dict:
                    for key in self.file_dict["CAL_FLAT_TW"]:
                        if not file_found:
                            file_name = key.split("/")[-2:]
                            print(
                                f"   - calib/{file_name[-2]}/{file_name[-1]} CAL_FLAT_TW"
                            )
    ...
    def util_calib(
        ...
        if not create_sof and not sof_file.exists():
            warnings.warn(
                f"The SOF file is not found at '{sof_file}' "
                "while 'create_sof' is set to False. "
                "Probably 'util_calib' has not been "
                "previously executed so forcing "
                "'create_sof' to True."
            )
                ...
                if calib_type in ["une", "fpet"]:
                    ...
                    if "UTIL_MASTER_FLAT" in self.file_dict:
                        for key, value in self.file_dict["UTIL_MASTER_FLAT"].items():
                            if not file_found:
                                # The FLAT correction is not applied when using
                                # the UTIL_MASTER_FLAT keyword in the SOF file
                                file_name = key.split("/")[-2:]
                                print(
    ...
    def util_trace(
        ...
        if not create_sof and not sof_file.exists():
            warnings.warn(
                f"The SOF file is not found at '{sof_file}' "
                "while 'create_sof' is set to False. "
                "Probably 'util_trace' has not been "
                "previously executed so forcing "
                "'create_sof' to True."
            )
    ...
    def util_slit_curv(
        ...
        if not create_sof and not sof_file.exists():
            warnings.warn(
                f"The SOF file is not found at '{sof_file}' "
                "while 'create_sof' is set to False. "
                "Probably 'util_slit_curv' has not been "
                "previously executed so forcing "
                "'create_sof' to True."
            )
    ...
    def util_extract(
        ...
        if not create_sof and not sof_file.exists():
            warnings.warn(
                f"The SOF file is not found at '{sof_file}' "
                "while 'create_sof' is set to False. "
                "Probably 'util_extract' has not been "
                "previously executed so forcing "
                "'create_sof' to True."
            )
    ...
    def util_normflat(self, verbose: bool = False, create_sof: bool = True) -> None:
        ...
        if not create_sof and not sof_file.exists():
            warnings.warn(
                f"The SOF file is not found at '{sof_file}' "
                "while 'create_sof' is set to False. "
                "Probably 'util_normflat' has not been "
                "previously executed so forcing "
                "'create_sof' to True."
            )
    ...
    def util_genlines(self, verbose: bool = False, create_sof: bool = True) -> None:
        ...
        if not create_sof and not sof_file.exists():
            warnings.warn(
                f"The SOF file is not found at '{sof_file}' "
                "while 'create_sof' is set to False. "
                "Probably 'util_genlines' has not been "
                "previously executed so forcing "
                "'create_sof' to True."
            )
    ...
    def util_wave(
        ...
        # Flag for re-using higher degrees of first guess
        # with cross-correlation done by EsoRex
        ...
        if not create_sof and not sof_file.exists():
            warnings.warn(
                f"The SOF file is not found at '{sof_file}' "
                "while 'create_sof' is set to False. "
                "Probably 'util_wave' has not been "
                "previously executed so forcing "
                "'create_sof' to True."
            )
    ...
    def obs_staring(
        ...
        print(f"\nNumber of exposures: {science_idx.size}")
        ...
        for i, i_row in enumerate(self.header_data.index[science_idx], start=1):
            # skip existing reduced files
            if check_existing and i < start:
                continue
            ...
            if not create_sof and not sof_file.exists():
                warnings.warn(
                    f"The SOF file is not found at '{sof_file}' "
                    "while 'create_sof' is set to False. "
                    "Probably 'obs_staring' has not been "
                    "previously executed so forcing "
                    "'create_sof' to True."
                )
            ...
            if create_sof:
                # Creating SOF file for each frame
    ...
    def obs_nodding(
        ...
        print(f"Correct bad pixels: {correct_bad_pixels}")
        print(f"Extraction required: {extraction_required}")
        print(f"Check existing: {check_existing}")
        print(f"Verbose: {verbose}")
        print(f"Create SOF: {create_sof}")
        ...
        print(f"\nNumber of exposures at nod A: {nod_a_count}")
        ...
        for i_row in self.header_data.index[nod_a_exp]:
            ...
            if not create_sof and not sof_file.exists():
                warnings.warn(
                    f"The SOF file is not found at '{sof_file}' "
                    "while 'create_sof' is set to False. "
                    "Probably 'obs_nodding' has not been "
                    "previously executed so forcing "
                    "'create_sof' to True."
                )
            ...
            if create_sof:
                ...
                if not file_found:
                    url = (
                        ...
                        f"pycrires/{science_wlen}_tw.fits"
                    )
                    ...
                    print(
                        f"   - product/obs_nodding/{science_wlen}_tw.fits UTIL_WAVE_TW"
                    )
                    sof_open.write(f"{tw_file} UTIL_WAVE_TW\n")
                    file_found = True
    ...
    def obs_nodding_irregular(
        ...
        if len(science_idx) == 0:
            raise RuntimeError(
                "Cannot run obs_nodding_irregular: there are no SCIENCE frames"
            )
            ...
            if check_existing and os.path.exists(output_file):
                ...
                if not create_sof and not sof_file.exists():
                    warnings.warn(
                        f"The SOF file is not found at '{sof_file}' "
                        "while 'create_sof' is set to False. "
                        "Probably 'obs_nodding_irregular' has not "
                        "been previously executed so forcing "
                        "'create_sof' to True."
                    )
                    ...
                    if "UTIL_MASTER_FLAT" in self.file_dict:
                        for key in self.file_dict["UTIL_MASTER_FLAT"]:
                            if not file_found:
                                ...
                                print(
                                    # f"   - calib/{file_name[-2]}/{file_name[-1]} UTIL_MASTER_FLAT"
                                    f"   - calib/{file_name[-2]}/{file_name[-1]} CAL_MASTER_FLAT"
                                )
                                # sof_open.write(f"{key} UTIL_MASTER_FLAT\n")
    ...
    def correct_wavelengths_2d(
        ...
        can be applied to 2D extracted spectra if the spatial dimension
        needs to be maintained. The ``accuracy`` can be iteratively
        ...
        if collapse_exposures:
            ...
            for file_item in fits_files:
                with fits.open(file_item) as hdu_list:
                    tot_flux += (
                        hdu_list["SPEC"].data / (hdu_list["ERR"].data + 1.0) ** 2
                    )
                    normalization += 1.0 / (hdu_list["ERR"].data + 1.0) ** 2

            tot_flux = tot_flux / normalization

            # Loop only over the first FITS file
        ...
        for fits_file in loop_files:
            ...
            current_wavel = hdu_list["WAVE"].data

            n_det = hdu_list["SPEC"].data.shape[0]
            n_order = hdu_list["SPEC"].data.shape[1]

            print(" [DONE]")

            if create_plots:
                fig, axes = plt.subplots(
                    n_order, n_det, figsize=(9, 15), sharex=False, sharey=False
                )
            ...
            for det_idx in range(n_det):
                ...
                for order_idx in np.arange(n_order):
                    ...
                    if collapse_exposures:
                        # Use the weighted combination of all spectra
                        spec_2d = tot_flux[det_idx, order_idx]
                    else:
                        # Read the 2D sepctrum of the current FITS file
                        spec_2d = hdu_list["SPEC"].data[det_idx, order_idx]

                    if collapse_spatially:
                        # Optionally sum over the spatial dimension
                        # to enhance the SNR of the telluric lines

                        # List with 1 array that contains the mean
                        # wavelength of the current solution
                        wavel_list = [np.nanmean(wavel_2d, axis=0)]

                        # Index of the central row
                        cent_idx = spec_2d.shape[0] // 2 + 1

                        # Select the central 13 rows
                        spec_2d = spec_2d[cent_idx - 6 : cent_idx + 7]

                        # Check if the sum along the wavelength
                        # dimension is zero for any of the 13 rows
                        if 0.0 in np.nansum(spec_2d, axis=1):
                            # Create list with 1 array spectrum
                            # containing only zeros
                            spec_list = [np.zeros(spec_2d.shape[1])]

                        else:
                            # Normalize each row in the 2D
                            # spectrum by the sum of the row
                            norm_spec = (
                                spec_2d / np.nansum(spec_2d, axis=1)[:, np.newaxis]
                            )

                            # Create a list with 1 array spectrum
                            # as the median of the normalized rows
                            spec_list = [np.nanmedian(norm_spec, axis=0)]

                    else:
                        # Or use the current 2D wavelength solution
                    ...
                    if template_std < minimum_strength:
                        # There are not sufficient telluric lines
                        # to do the wavelength recalibration
                        ...
                        warnings.warn(
                            ...
                            "The wavelength range of the CRIRES spectra "
                            f"is {np.min(wavel_2d):.2f} to "
                            f"{np.max(wavel_2d):.2f} nm. The wavelength "
                            "mask to match with the SkyCalc model "
                            f"contains {np.sum(wl_mask)} values. "
                            "The wavelength solution from EsoRex will "
                            "be adopted."
                        )

                        # Check for collapse_spatially is not needed
                        # since the original wavelength solution will
                        # be stored in the output files.

                        if collapse_exposures:
                            # Save the original wavelengths to
                            # all files for the current order
                            ...
                            # Save the original wavelengths only to
                            # current FITS file for the current order

                            out_file = output_dir / (
                                Path(fits_file).stem.replace("_corr", "") + "_corr.fits"
                            )

                            if det_idx == 0 and order_idx == 0:
                                # Open the current FITS file for the
                                # first detector and first order.
                                # The content will be stored in the
                                # output FITS file.
                                save_hdu_list = fits.open(fits_file)
                            else:
                                # Open the output FITS file, which
                        ...
                        # There are sufficient telluric lines
                        # to do the wavelength recalibration

                        # Iterate of the rows of the 2D spectrum
                        # With collapse_exposures=True, spec_list and
                        # wavel_list contain 1 array item each
                        for row, (spec, wavel) in enumerate(zip(spec_list, wavel_list)):
                            ...
                            if collapse_exposures:
                                # Save the updated wavelengths to
                                # all files for the current order
                                ...
                                for save_file in fits_files:
                                    ...
                                    if collapse_spatially:
                                        # Use the wavelength solution
                                        # for all rows of the order
                                ...
                                # Save the updated wavelengths only to
                                # the current file for the current order

                                out_file = output_dir / (
                                    Path(fits_file).stem.replace("_corr", "")
                                    + "_corr.fits"
                                )
