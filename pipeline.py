...
class Pipeline:
    ...
    def _create_config(
        ...
        if not os.path.exists(config_file):
            ...
            if eso_recipe == "cr2res_cal_dark":
                ...
                config_text = config_text.replace(
                    "cr2res.cr2res_obs_nodding.extract_height=-1",
                    "cr2res.cr2res_obs_nodding.extract_height=11",
                )
                config_text = config_text.replace(
                    "cr2res.cr2res_obs_nodding.extract_smooth_slit=1.0",
                    "cr2res.cr2res_obs_nodding.extract_smooth_slit=2.0",
                )
                config_text = config_text.replace(
                    "cr2res.cr2res_obs_nodding.extract_smooth_spec=8e-08",
                    "cr2res.cr2res_obs_nodding.extract_smooth_spec=2.0e-7",
                )
    ...
    def obs_nodding3(
        ...
        for i_row in science_idx:
            ...
            if os.path.exists(output_file) and continue_reduction:
    ...
    def obs_nodding2(
        ...
        science_wlen = self.header_data["INS.WLEN.ID"][science_idx[0]]
        science_dit = self.header_data["DET.SEQ1.DIT"][science_idx[0]]
        N_expo = int(self.header_data["SEQ.NEXPO"][science_idx[0]])
        ...
        # Iterate over nod A exposures
        i_rows = self.header_data.index[nod_a_exp][::N_expo]
        for i_row in i_rows:
            if os.path.exists(pathlib.Path(output_dir / f"cr2res_obs_nodding_combinedA_{count_exp:03d}.fits")):
                print(f'Already done nod pair #{count_exp+1}/{indices.sum()//2}')
            else:
                print(
                ...
                file_0 = self.header_data["ORIGFILE"][i_row]
                a_files = [self.header_data['ORIGFILE'][i_row+i_expo] for i_expo in range(N_expo)]

                if self.header_data["SEQ.NODPOS"][i_row + N_expo] == "B":
                    # AB pair, so using the next exposure for B
                    file_1 = self.header_data["ORIGFILE"][i_row + N_expo]
                    b_files = [self.header_data['ORIGFILE'][i_row + N_expo + i_expo] for i_expo in range(N_expo)]
    ...
    @typechecked
    def xcor_wavelength_solution_2nd_order(
        self,
        spec: np.ndarray,
        wavel: np.ndarray,
        ...
        nans = np.isnan(spec) + (spec<0.)
        window_length = min([201, 2*(np.sum(~nans)//2)-1])
        continuum = signal.savgol_filter(
            spec[~nans], window_length=window_length,
            polyorder=2, mode='interp')
        
        #spec_flat = spec[~nans] - continuum
        #spec_flat = spec[~nans]/spec[~nans].max()
        spec_flat = spec[~nans]-continuum
        #outliers = np.abs(spec_flat)>(10*np.nanstd(spec))
        ...
        #da = 3e-4
        #dc = 1e-4
        N_a = int(np.ceil(0.02/ da) // 2 * 2 + 1)
        N_b = int(np.ceil(0.1/ db) // 2 * 2 + 1)
        N_c = int(np.ceil(0.001 / dc) // 2 * 2 + 1)
        N_a, N_b, N_c = 51, 51, 51

        b_grid = np.linspace(
            -db*N_b/2, db*N_b/2, N_b)[:, np.newaxis, np.newaxis, np.newaxis]
        a_grid = np.linspace(
            1-da*N_a/2, 1+da*N_a/2, N_a)[np.newaxis, :, np.newaxis, np.newaxis]
        c_grid = np.linspace(
                -dc*N_c/2, dc*N_c/2, N_c)[np.newaxis, np.newaxis, :, np.newaxis]
    ...
    def xcor_wavelength_solution(
        ...
        spec = spec[10:-10]
        wavel = wavel[10:-10]
        nans = np.isnan(spec) + (spec<0.01*np.nanmedian(spec))
        continuum = signal.savgol_filter(
            spec[~nans], window_length=window_length,
            polyorder=2, mode='interp')
        
        spec_flat = spec[~nans] / continuum - 1
        spec_flat = spec[~nans] / spec[~nans].max() - 1
        outliers = np.abs(spec_flat)>(8*np.nanstd(spec))
        spec_flat[outliers]=0
        ...
        if return_cross_corr:
            return  cross_corr, (opt_b, opt_a, 0)
        else:
            return (opt_b, opt_a, 0)
    ...
    def correct_wavelengths(
        ...
        if input_folder == "obs_nodding":
            ...
            import glob
            input_files = str(self.product_folder) + f"/obs_nodding/cr2res_obs_nodding_extracted{nod_ab}_*.fits"
                ...
                for order, spec_name in enumerate(spec_orders):
                    ...
                    if template_std > minimum_strength:
                        ...
                        if sum_over_time:
                            ...
                            cross_corr, (opt_b, opt_a, opt_c) = self.xcor_wavelength_solution_2nd_order(
                                tot_flux, wavel, transm_spec, accuracy, 
                                window_length, return_cross_corr=True)
                            ...
                            # Calculate the cross-correlation
                            # between data and template
                            cross_corr, (opt_b, opt_a, opt_c), opt_idx = self.xcor_wavelength_solution_2nd_order(
                        ...
                        if create_plots:
                            ...
                            dlam = (wavel[-1]-np.mean(wavel))
                            plt.imshow(
                                cross_corr[:,:, cross_corr.shape[-1]//2],
                                extent=[-N_points/2*accuracy, N_points/2*accuracy,1-N_points/2*accuracy/dlam, 1+N_points/2*accuracy/dlam],
                                origin="lower",
                                aspect="auto",
                            )
                            plt.colorbar()
    ...
    def correct_wavelengths_2d(
        ...
        for file_i, fits_file in enumerate(loop_files):
                ...
                for order_idx in np.arange(7):
                    ...
                    if not template_std > minimum_strength:
                        ...
                        for row, (spec, wavel) in enumerate(zip(spec_list, wavel_list)):
                            ...
                            cross_corr, opt_p, opt_idx = self.xcor_wavelength_solution_2nd_order(
                                spec, wavel, transm_spec, accuracy, window_length, return_cross_corr=True)
                            ...
                            if sum_over_time and sum_over_spatial_dim:
                                # Save corrected wavelengths to all files for this order
                                for save_file in fits_files:
                        ...
                        if create_plots:
                            ...
                            plt.imshow(
                                cross_corr[opt_idx[0], :, :],
                                #extent=[-(N_points+1)/2*accuracy, (N_points+1)/2*accuracy,1-(N_points+1)/2*accuracy/dlam, 1+(N_points+1)/2*accuracy/dlam],
                                extent=[-(N_points+1)/2*accuracy/dlam**2, (N_points+1)/2*accuracy/dlam**2,
                                        1-(N_points+1)/2*accuracy/dlam, 1+(N_points+1)/2*accuracy/dlam],
                                origin="lower",
                                aspect="auto",
                                interpolation='none'
                            )
                            ...
                            plt.axvline(opt_p[2], ls="--", color="white")
            ...
            if create_plots:
                ...
                plt.tight_layout()
                plt.savefig(f"{output_dir}/correlation_map_{nod_ab}_{accuracy}.png")
